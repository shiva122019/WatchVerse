...
// function CenterControlButton({ onClick, children, size = 48, disabled, ariaLabel }) {
...
//       className="relative flex items-center justify-center overflow-hidden rounded-full bg-black/35 text-white shadow-md transition-colors hover:bg-black/50 disabled:opacity-40"
//     >
//       {ripples.map((r) => (
//         <motion.span
//           key={r.id}
...
//   // New: auto-hide the control chrome after AUTO_HIDE_MS of no pointer
//   // activity, but only while actually playing — while paused the poster
//   // cover already owns the screen, so leaving controls up is expected.
//   useEffect(() => {
...
//     clearTimeout(hideTimeoutRef.current);
//     if (syncedPlaying && !settingsOpen) setControlsVisible(false);
//   };
...
//   const showChrome = controlsVisible || !syncedPlaying;
...
//       {/* Full-frame tap target sitting under the cover. This only ever
//           receives clicks once the cover (below) is hidden — i.e. while
//           actively playing — so it's the single click-to-pause surface.
//           It must NOT sit on top of the cover, or you get a double
//           play/pause target (cover's own button + this one) stacked on
//           top of each other, which is what was happening before. */}

//       {/* Cover — opaque any time we're not playing, so none of YouTube's
//           own paused-state UI (title, channel, logo, endscreen, captions)
//           is ever visible. No blur and no play icon of its own anymore —
//           it's a plain background layer only; the center transport controls
//           below are the single, merged play/pause control for the player. */}
...
//               className="pointer-events-auto absolute inset-x-0 top-0 flex items-center justify-between gap-3 bg-gradient-to-b from-black/70 via-black/20 to-transparent p-4"
//             >
...
//                 className="pointer-events-auto rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-white/90 shadow-lg backdrop-blur-xl transition hover:bg-white/15"
//               >
//                 Search movie
//               </button>
//             </motion.div>
...
//             >
//               <div className="flex items-center gap-1 rounded-full border border-white/15 bg-white/10 px-2 py-1.5 shadow-lg backdrop-blur-xl">
//                 <button
//                   onClick={toggleMute}
//                   disabled={!ready}
...
function Timeline({ currentTime, duration, onScrub, onCommit, disabled }) {
  ...
  const shown = dragTime ?? currentTime;
  const pct = duration
    ? Math.min(100, Math.max(0, (shown / duration) * 100))
    : 0;
          ...
          <div className="max-h-56 overflow-y-auto">
            {PLAYBACK_RATES.map((rate) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-sm text-white/90 transition hover:bg-white/10"
              >
                <span className={rate === playbackRate ? "text-[#5CF2E3]" : ""}>
                  {rate === 1 ? "Normal" : `${rate}x`}
                </span>
                {rate === playbackRate && (
                  <Check className="h-4 w-4 text-[#5CF2E3]" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="p-1.5">
          ...
          <div className="max-h-56 overflow-y-auto">
            {QUALITY_ORDER.filter(
              (q) => q === "auto" || availableQualities.includes(q),
            ).map((q) => (
              <button
                key={q}
                onClick={() => onQualityChange(q)}
                className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-sm text-white/90 transition hover:bg-white/10"
              >
                <span className={q === quality ? "text-[#5CF2E3]" : ""}>
                  {QUALITY_LABELS[q]}
                </span>
                {q === quality && <Check className="h-4 w-4 text-[#5CF2E3]" />}
              </button>
