...
import { Pencil, Settings, LayoutDashboard, PlayCircle, Loader2 } from "lucide-react";
import api from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import { toast } from "sonner";

export default function ProfileActions({ role, isOwnProfile, onViewContent, profileUsername }) {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    api.get(`/followers/${profileUsername}`)
      .then((res) => {
        if (isMounted) {
    ...
    <div className="mt-6 flex flex-wrap gap-3 relative z-10">
      {isOwnProfile ? (
        ...
        <>
          ...
          >
            {loading ? <Loader2 size={16} className="animate-spin" /> : isFollowing ? "Following" : "Follow"}
          </motion.button>
          <motion.button
            type="button"
